            ...
            for j in range(self.directions):
                ...
                if 0 <= v_x < self.N and 0 <= v_y < self.N and self.graph[v_x][v_y] == 1 and not visited[v_x][v_y]:
                    ...
                    if v_x == self.goal.x and v_y == self.goal.y:  # Goal check
                        ...
                        print(f"Moving to goal -> ({v_x}, {v_y})")
                        return
                    visited[v_x][v_y] = True
                    move_direction = self.get_move_direction(u.x, u.y, v_x, v_y)
                    print(f"{move_direction} -> ({v_x}, {v_y})")
                    child = Node(v_x, v_y, v_level)
                    q.append(child)

    def get_move_direction(self, prev_x, prev_y, curr_x, curr_y):
        if curr_x > prev_x:
            return "Moving Down"
        elif curr_x < prev_x:
            return "Moving Up"
...
def main():
    ...
    if bfs.found:
        print("\nGoal found!")
        print(f"Number of moves required to reach the goal = {bfs.goal_level}")
    else:
